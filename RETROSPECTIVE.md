...
| 결정 | 선택 | 이유 |
|------|------|------|
| 실행 방식 | 완전 로컬 | 개인정보 보호 (이력서, 연락처 등) |
| LLM | **로컬 LLM만** (Ollama / LM Studio) | 비용 zero, 데이터 외부 전송 없음 |
...
Ollama를 쓰면 완전 무료 + 완전 로컬로 전체 기능을 사용할 수 있다.

### "외부 API는 쓰지 않는다"

처음에는 Anthropic/OpenAI API도 지원하려 했지만, 고민 끝에 **로컬 LLM 전용**으로 결정했다.

이유:
1. **이력서/경력기술서는 개인정보 덩어리** — 외부 API로 보내고 싶지 않음
2. **API 비용 부담** — 서류 하나 다듬는데 $0.05씩 나가는 건 말이 안 됨
3. **Ollama 성능이 충분** — llama3, qwen2.5 등으로 한국어 서류 작성 충분
4. **오프라인에서도 동작** — 카페에서 취업 준비하다가 인터넷 끊겨도 작동
...
- LLM 클라이언트 추상화 (Ollama / OpenAI 호환 로컬 서버)
...
### Phase 5: 채용 사이트 어댑터 추가 (5개 사이트 완성)
...
- **점핏 어댑터** (내부 API)
  - 기술 태그 기반 검색
  - API 실패 시 웹 검색 폴백
- **그룹바이 어댑터** (Next.js SSR 파싱)
  - `server-sitemap.xml`에서 position ID 수집
  - `__NEXT_DATA__` JSON 파싱으로 구조화 데이터 추출
  - 스타트업 전문 채용 플랫폼
...
| 플랫폼 | 판정 | 이유 |
|--------|------|------|
| **그룹바이** | ✅ 추가 | 스타트업 전문, Next.js SSR로 깔끔한 데이터 추출 가능 |
| **혁신의숲** | ❌ 제외 | 채용 플랫폼이 아님. 스타트업 분석 플랫폼이고 채용은 원티드 연동 (중복) |
| **위시캣** | ❌ 제외 | 프리랜서 프로젝트 매칭. 정규직 채용공고 스키마와 구조 불일치 |
| **로켓펀치** | ❌ 제외 | 서비스 종료 |

### LLM 정책 변경: 외부 API → 로컬 전용

초기에는 Anthropic/OpenAI API도 지원했으나, **로컬 LLM 전용**으로 전환했다.

- `@anthropic-ai/sdk` 의존성 제거
- LLM 클라이언트를 Ollama + OpenAI 호환 로컬 서버만 지원하도록 변경
- `.env.example`에서 외부 API 키 항목 제거
...
| `resume_tailor` | ✅ LLM 기반 | ✅ (로컬) |
| `resume_export` | ✅ 템플릿 + LLM 선택적 | 선택적 |
| `coverletter_brainstorm` | ✅ LLM 기반 | ✅ (로컬) |
| `coverletter_generate` | ✅ LLM 기반 | ✅ (로컬) |
| `portfolio_reorder` | ✅ LLM 기반 | ✅ (로컬) |
| `application_create` | ✅ 완전 동작 | ❌ |
| `application_update_status` | ✅ 완전 동작 | ❌ |
| `application_list` | ✅ 완전 동작 | ❌ |
| `interview_prepare` | ✅ LLM 기반 | ✅ (로컬) |
...
- 점핏: 비공식 내부 API + 웹 폴백
- 그룹바이: Next.js SSR → `__NEXT_DATA__` JSON 파싱
...
- 로컬 LLM 모델별 성능 벤치마크 (llama3 vs qwen2.5 vs gemma2)
...
4. **로컬 LLM이 생각보다 쓸만하다** — Ollama + qwen2.5면 한국어 서류 작성 충분히 된다.
5. **어댑터 패턴의 위력** — 사이트마다 API가 달라도, 추상화 한 번이면 나머지 코드는 건드릴 필요 없다.
6. **MCP가 개인 도구 개발에 적합하다** — UI 없이 Claude에서 바로 도구를 쓸 수 있는 건 강력하다.
