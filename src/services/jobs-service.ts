...
export class JobsService {
  ...
  async searchJobs(params: SearchJobsParams) {
    ...
    const rankedJobs = this.rankJobs(Array.from(jobsByKey.values()), params.keywords);
    const sourcesResultCount = this.buildSourceResultCount(rankedJobs, params, requestedSources);
    this.addZeroResultWarnings(warnings, sourcesResultCount, zeroResultReasons, params, requestedSources);
    const limitedJobs = rankedJobs.slice(0, params.limit);
