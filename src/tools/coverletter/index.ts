...
프로젝트:
${profile.projects.map(p => `- ${p.name}: ${p.description} / 성과: ${p.achievements.join(', ')}`).join('\n')}
도메인: ${profile.domains.join(', ')}

[문항]
${params.questions.map((q, i) => `${i + 1}. ${q.question_text}${q.max_length ? ` (${q.max_length}자)` : ''}`).join('\n')}`,
          }],
          temperature: 0.5,
        });
...
6. 톤: ${toneGuide[params.tone || 'professional']}
...
[프로필]
경력: ${profile.total_experience_years}년
프로젝트: ${profile.projects.map(p => `${p.name}: ${p.description} (성과: ${p.achievements.join(', ')})`).join('\n')}
