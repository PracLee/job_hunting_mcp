...
이 프로젝트의 기획 배경과 개발 회고는 아래 글에서 확인하실 수 있습니다:
- [📝 프로젝트 회고록 (Tistory)](https://mrgoga.tistory.com/99)

사용 중 발견한 **버그, 부족한 점, 개선 아이디어** 등 피드백이 있다면 위 블로그의 댓글로 자유롭게 남겨주세요!
...
**프로필 스키마 확장**
- `total_experience_months` 필드 추가 — 경력을 연 단위 외 월 단위로도 저장
- `user_confirmed_skills` / `user_rejected_skills` 필드 추가 — 스킬 확정/거부 워크플로우 지원
- 기존 DB 자동 마이그레이션 (ALTER TABLE)

**이력서 파서 정확도 전면 개선**
- 섹션 헤더 판단 강화: `기술 스택: Python, ...` 같이 `:` 뒤 내용이 있는 줄을 헤더로 잘못 인식하던 버그 수정 → 프로젝트 섹션 중간 절단 현상 해소
- 경력 연차 계산: `총 N년 N개월` 패턴 1순위 적용, `months` 값 동시 반환
- 이름 추출: `이병재 경력` 같은 채용사이트 복합 형식에서 이름 추출 가능
- 프로젝트 제목 감지: bullet(`-`, `•`) 으로 시작하는 줄을 프로젝트 제목으로 오인하던 버그 수정
- 기술스택 추출: `기술` 단독 키워드 매치 방지 → `기술 스택:` 명시 형태만 허용
- 학력 전공 파싱: `학과?` 패턴이 학교명(`협성대학교`)을 전공으로 오인하던 버그 수정
- 프로젝트 유효성 필터 추가: `학력`, `보유 기술` 등 섹션 헤더 및 개인정보 블록이 프로젝트로 등록되던 문제 해소
